...
def get_data():
    ...
    data = pd.read_csv("df_filtered60.csv")
...
body {
    ...
    type_cat = st.sidebar.radio('Quel type de recommendation voulez-vous consulter ?',('Famille Produit', 'TOP 100'), key='hello')
...
if (type_cat=='Famille Produit'):
    ...
    data['Count']= data['Count'].apply(lambda x : round(x*100,2))
    for im in range(len(data.Link)):
        try:
            #pic ='"%s"'%data.loc[im,'Image']
            ...
            st.markdown(str('Importance Score : ' + str(data.loc[im,'Count'] ) + str('%')))
            st.markdown(str('Source la plus représentée: ' + data.loc[im,'Source']))
            count+=1
        except:
    ...
    st.title('TOP 100')
    data2=pd.read_csv('Best Product.csv', delimiter=';')
    for im in range(len(data2.Thumbnail)):
        try:
            #pic ='"%s"'%data.loc[im,'Image']
            #urllib.request.urlretrieve(data.loc[im,'Link'], "local-filename.jpg")
            response = requests.get(data2.loc[im,'Thumbnail'])
            img = Image.open(BytesIO(response.content)).convert('RGB')
            #img = Image.open(urllib.request.urlretrieve(data.loc[im,'Link'], "local-filename.jpg")).convert('RGB').save('new.jpeg')
            st.title(str('Ranking : ') + str(count+1))
            st.markdown('<style>h1{color: grey;}</style>', unsafe_allow_html=True)
            ...
            #st.image("local-filename.jpg",width=max_width, use_column_width=False)#, caption= str('Product : ' + data.loc[im,'Cluster']) + ' \n Produit : ' + str(data.loc[im,'Cluster FR'])+ ' \n Importance Score ' + str(data.loc[im,'Count']))
            st.markdown( str('Product (EN) : ' + data2.loc[im,'Title']))
            st.markdown(str('Produit (FR) : ' + data2.loc[im,'Title']))
